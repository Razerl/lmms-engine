...
class TestQwen3_VL(TestCase):
    ...
    def test_text_train_fsdp2(self, temp_dir, nproc_per_node):
        ...
        script_path = os.path.join(os.path.dirname(__file__), "train_qwen3_vl.py")
    ...
    @with_temp_dir
    @with_multi_gpu_training
    def test_text_train_fsdp2_ema(self, temp_dir, nproc_per_node):
        """Test Qwen3-VL training with FSDP2 + EMA using torchrun subprocess."""

        # Path to the training script
        script_path = os.path.join(os.path.dirname(__file__), "train_qwen3_vl_ema.py")
