...
usage() {
    cat <<'EOF'
Usage: sh examples/qwen3_vl/qwen3_vl_2b_train.sh [RUN_NAME] [WANDB_API_KEY] [NPROC_PER_NODE] [WARMUP_STEPS] [MAX_STEPS] [PACKING_LENGTH]

You can also pass values through environment variables:
  RUN_NAME=exp-name WANDB_API_KEY=... NPROC_PER_NODE=8 WARMUP_STEPS=500 MAX_STEPS=5000 PACKING_LENGTH=40000 sh examples/qwen3_vl/qwen3_vl_2b_train.sh
EOF
}
...
PROCESSOR_NAME="/home/rzli/data/data_ssd_smb/models/QwenVL/Qwen3-VL-2B-Instruct"
MODEL_PATH="/home/rzli/data/data_ssd_smb/models/QwenVL/Qwen3-VL-2B-Instruct"
...
NUM_TRAIN_EPOCHS=1
RUN_NAME="${1:-${RUN_NAME:-Qwen3VL-2B-ALL-SFT-test}}"
...
if [ -z "${WANDB_API_KEY}" ]; then
    ...
    echo "Usage: sh examples/qwen3_vl/qwen3_vl_2b_train.sh ${RUN_NAME} <WANDB_API_KEY> [NPROC_PER_NODE] [WARMUP_STEPS] [MAX_STEPS] [PACKING_LENGTH]" >&2
    echo "Or: RUN_NAME=${RUN_NAME} WANDB_API_KEY=<key> sh examples/qwen3_vl/qwen3_vl_2b_train.sh" >&2
    exit 1
fi
